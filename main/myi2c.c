...
esp_err_t i2c_WriteCommand(i2c_port_t i2c_num, uint8_t deviceAddr, uint8_t command) {
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, deviceAddr << 1, I2C_ACK_CHECK_EN);
...
esp_err_t i2c_WriteBuffer(i2c_port_t i2c_num, uint8_t deviceAddr, uint8_t* pBuffer, int numBytes) {
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, deviceAddr << 1, I2C_ACK_CHECK_EN);
	for (int n = 0; n < numBytes; n++) {
		i2c_master_write_byte(cmd, pBuffer[n], I2C_ACK_CHECK_EN);
		}
    i2c_master_stop(cmd);
    int ret = i2c_master_cmd_begin(i2c_num, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
