...
uint8_t SCCB_ReadBytes(uint8_t slv_addr, uint8_t reg, uint8_t* pBuf, int numBytes)
{
    ...
    if (rc != 0) {
        ...
        rc = twi_readFrom(slv_addr, pBuf, numBytes, true);
        if (rc != 0) {
            data=0xFF;
        }
    }
...
uint8_t SCCB_WriteBytes(uint8_t slv_addr, uint8_t* pBuf, int numBytes)
{
    ...
    __disable_irq();
    if(twi_writeTo(slv_addr, pBuf, numBytes, true) != 0) {
        ret=0xFF;
    }
    __enable_irq();
    if (ret != 0) {
        printf("SCCB_WriteBytes failed\n");
    }
    return ret;
}
...
uint8_t SCCB_Write2(uint8_t slv_addr, uint8_t reg, uint8_t data1, uint8_t data2)
{
    ...
    __enable_irq();
    if (ret != 0) {
        printf("SCCB_Write2[%02x]=%02x%02x failed\n", reg, data1, data2);
    }
    return ret;
}
...
{
    uint8_t ret=0;
    uint8_t buf[] = {reg, data1, data2, data3, data4};

    __disable_irq();
    if(twi_writeTo(slv_addr, buf, 5, true) != 0) {
        ret=0xFF;
    }
    __enable_irq();
    if (ret != 0) {
        printf("SCCB_Write4[%02x] failed\r\n", reg);
    }
    return ret;
}
